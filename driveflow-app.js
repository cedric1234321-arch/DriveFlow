...
function syncHistoricalAutoSessions(){
  ...
  const manualSessions=sessions.filter(s=>s.autoHistorical!==true);
  const manualDates=new Set(manualSessions.map(s=>s.date));
  ...
  function addHistoricalRecord(r,platform){
    ...
    if(manualDates.has(r.businessDate))return;
  ...
  // Historical sessions are common to both apps: merge Uber + Deliveroo order starts.
  ...
  byDate.forEach((items,date)=>{
    ...
    items.forEach(item=>{
      if(previousMinute!==null && item.minute-previousMinute>=HISTORICAL_SESSION_GAP_MIN){
    ...
    groups.forEach(group=>{
      ...
      const endMinute=last.minute+HISTORICAL_SESSION_END_BUFFER_MIN;
      const start=clockFromBusinessMinute(startMinute),end=clockFromBusinessMinute(endMinute);
      const platforms=[...new Set(group.map(x=>x.platform))];
      autoSessions.push({
        id:`autoHist:${date}:${first.record.id}`,
        date,
        type:historicalSessionType(start),
        start,
        end,
        pauseStart:"",
        pauseEnd:"",
        ...
        autoHistorical:true,
        autoStartMinute:startMinute,
        autoEndMinute:endMinute,
        autoOrderCount:group.reduce((a,x)=>a+n(x.record.orderCount),0),
        autoPlatforms:platforms
      });
    });
  });

  sessions=[...manualSessions,...autoSessions];
...
function importDeliveroo(rows){
  ...
  deliverooOrders.sort((a,b)=>a.timestamp.localeCompare(b.timestamp));
...
/* Swipe navigation between main tabs — V4.0.5
   - horizontal swipe only
   - keeps vertical scrolling untouched
   - ignores forms/buttons/date controls/sheets/compare page
   - avoids iOS edge-back gesture */
(()=>{
  const VIEW_ORDER=["today","week","stats","history","settings"];
  let startX=0,startY=0,startTime=0,startTarget=null;

  const interactiveSelector=[
    "button","a","input","select","textarea","label",
    ".sheet",".compare-page",".date-switcher",".period-nav",
    ".bottom-nav",".fab"
  ].join(",");

  function currentView(){
    const active=document.querySelector(".bottom-nav button.active");
    return active?.dataset.view || "today";
  }

  function swipeBlocked(target,x){
    if(document.querySelector(".sheet.open")) return true;
    if(document.querySelector("#comparePage.open")) return true;
    if(target?.closest?.(interactiveSelector)) return true;
    if(x<28 || x>window.innerWidth-28) return true; // preserve iOS edge gestures
    return false;
  }
  ...
  document.addEventListener("touchstart",e=>{
    ...
    const t=e.touches[0];
    if(swipeBlocked(e.target,t.clientX)){
      startTarget=null;
      return;
    }
    startX=t.clientX;
  ...
  document.addEventListener("touchend",e=>{
    if(!startTarget || !e.changedTouches.length) return;
    ...
    // Require a clear, intentional horizontal gesture.
    if(dt>700) return;
...
$("restoreJson").onclick=()=>$("restoreJsonInput").click();$("restoreJsonInput").onchange=async e=>{const f=e.target.files[0];if(!f)return;try{const d=JSON.parse(await f.text());if(d.version!==4||!Array.isArray(d.sessions))throw new Error();sessions=d.sessions;uberBatches=Array.isArray(d.uberBatches)?d.uberBatches:[];deliverooOrders=Array.isArray(d.deliverooOrders)?d.deliverooOrders:[];settings={...DEFAULTS,...(d.settings||{})};syncHistoricalAutoSessions();saveAll();applyTheme();renderAll();alert("Sauvegarde restaurée.")}catch{alert("Sauvegarde DriveFlow V4 invalide.")}e.target.value=""};
