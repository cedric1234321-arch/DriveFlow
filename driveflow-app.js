...
function syncHistoricalAutoSessions(){
  if(!Array.isArray(uberBatches)||!uberBatches.length)return {changed:false,created:0,days:0};
  ...
  uberBatches.forEach(r=>{
    if(!r||!r.businessDate||r.orderCount<=0)return;
    ...
    byDate.get(r.businessDate).push({record:r,minute});
  });
  ...
  byDate.forEach((items,date)=>{
    ...
    groups.forEach((group,index)=>{
      const first=group[0].record,last=group[group.length-1].record;
      const start=timeFromTimestamp(first.timestamp),end=timeFromTimestamp(last.timestamp);
      autoSessions.push({
        id:`autoHist:${date}:${first.id}`,
        date,
        ...
        note:`Session historique estimée automatiquement à partir des commandes Uber (écart de ${HISTORICAL_SESSION_GAP_MIN} min ou plus = nouvelle session).`,
        autoHistorical:true,
  ...
  const oldSignature=previousAuto.map(s=>`${s.id}|${s.start}|${s.end}`).sort().join(";");
  const newSignature=autoSessions.map(s=>`${s.id}|${s.start}|${s.end}`).sort().join(";");
